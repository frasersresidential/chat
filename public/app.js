...
const state = {
  ...
  pendingAttachments: [],
  canned: null,
  adspy: { tab: 'feed', competitorId: '', sort: 'longest', q: '' },
};
...
function render() {
  ...
  if (state.view === 'adspy') return renderAdSpy(main);
...
function adCard(ad) {
  ...
  return `<div class="ad-card ${ad.status !== 'active' ? 'inactive' : ''}" data-ad="${ad.id}">
    ${adThumb(ad)}
    <div class="ad-body">
      <div class="ad-top">
...
async function renderAdSpy(main) {
  let cfg, competitors;
  try {
    [cfg, competitors] = await Promise.all([api('/adspy/config'), api('/adspy/competitors')]);
  } catch (e) {
    main.innerHTML = `<div class="admin"><h2>🕵️ Ad Spy</h2><p class="muted">ต้องมีสิทธิ์ View Analytics (Owner / Admin / Manager) จึงจะดูได้</p></div>`;
    return;
  }
  const manage = can('manage_automation');
  const a = state.adspy;
  ...
  main.innerHTML = `<div class="admin adspy">
    <div class="report-toolbar">
      <h2 style="margin:0">🕵️ Facebook Ad Spy — สอดแนมโฆษณาคู่แข่ง</h2>
      <div class="report-actions">
        <span class="pill ${cfg.live ? 'role-agent' : ''}" title="${cfg.live ? 'ต่อกับ Meta Ad Library จริง' : 'โหมดตัวอย่าง (mock) — ใส่ META_AD_LIBRARY_TOKEN เพื่อดึงข้อมูลจริง'}">${cfg.live ? '🟢 LIVE · Ad Library' : '🟡 MOCK · ข้อมูลตัวอย่าง'}</span>
        ${manage ? '<button class="btn ghost" id="adRefreshAll">↻ อัปเดตทั้งหมด</button>' : ''}
      </div>
    </div>
    <p class="muted">ข้อมูลจาก <b>Meta Ad Library</b> สาธารณะ (ไม่ใช่การ scrape) — ดูครีเอทีฟ, CTA, แพลตฟอร์ม และ<b>ระยะเวลาที่รันโฆษณา</b> ซึ่งเป็นสัญญาณว่าแอดไหนน่าจะกำไร คลังโฆษณาไม่เปิดเผยงบ/ยอดวิวสำหรับแอดขายทั่วไป</p>

    <div class="card">
      <h3>🎯 คู่แข่งที่เฝ้าดู (${competitors.length})</h3>
      <div class="competitor-grid">
        ${competitors.map((c) => `<div class="competitor-chip ${a.competitorId === c.id ? 'active' : ''}" data-cmp="${c.id}">
          ...
          ${manage ? `<div class="cc-actions"><button class="btn ghost" data-cmp-refresh="${c.id}" title="อัปเดต">↻</button><button class="btn ghost" data-cmp-alerts="${c.id}" data-next="${c.alertsEnabled ? '0' : '1'}" title="เปิด/ปิดแจ้งเตือน">${c.alertsEnabled ? '🔔' : '🔕'}</button><button class="btn ghost" data-cmp-del="${c.id}" title="ลบ">✕</button></div>` : ''}
        </div>`).join('') || '<span class="muted">ยังไม่มีคู่แข่งในรายการ</span>'}
      </div>
      ${manage ? `<div class="form-grid" style="margin-top:12px">
        ...
        <div><label>ประเทศ</label><input id="cmpCountry" value="${esc(cfg.defaultCountry)}" style="width:80px" maxlength="2" /></div>
        <div><button class="btn" id="cmpAdd">+ เพิ่มคู่แข่ง</button></div>
      </div><div class="muted" id="cmpMsg" style="font-size:12px"></div>` : ''}
    </div>

    <div class="conv-tabs" id="adspyTabs">
      ${tabs.map(([k, label]) => `<button data-tab="${k}" class="${a.tab === k ? 'active' : ''}">${label}</button>`).join('')}
    </div>
    <div id="adspyBody"></div>
  </div>`;

  // Watchlist wiring
  main.querySelectorAll('[data-cmp]').forEach((el2) => el2.onclick = (e) => {
    if (e.target.closest('button')) return;
    a.competitorId = a.competitorId === el2.dataset.cmp ? '' : el2.dataset.cmp;
    renderAdSpy(main);
  });
  if (manage) {
    main.querySelectorAll('[data-cmp-del]').forEach((b) => b.onclick = async () => {
      if (!confirm('ลบคู่แข่งรายนี้และโฆษณาที่เก็บไว้?')) return;
      await api('/adspy/competitors/' + b.dataset.cmpDel, { method: 'DELETE' });
      if (a.competitorId === b.dataset.cmpDel) a.competitorId = '';
      renderAdSpy(main);
    });
    main.querySelectorAll('[data-cmp-refresh]').forEach((b) => b.onclick = async () => {
      b.textContent = '…';
      try { const r = await api('/adspy/competitors/' + b.dataset.cmpRefresh + '/refresh', { method: 'POST' }); $('#cmpMsg') && ($('#cmpMsg').textContent = `✓ อัปเดตแล้ว — โฆษณาใหม่ ${r.newAds} ชิ้น`); }
      catch (e) { alert(e.message); }
      renderAdSpy(main);
    });
    main.querySelectorAll('[data-cmp-alerts]').forEach((b) => b.onclick = async () => {
      await api('/adspy/competitors/' + b.dataset.cmpAlerts, { method: 'PUT', body: JSON.stringify({ alertsEnabled: b.dataset.next === '1' }) });
      renderAdSpy(main);
    });
    if ($('#adRefreshAll')) $('#adRefreshAll').onclick = async () => {
      $('#adRefreshAll').textContent = '… กำลังอัปเดต';
      try { const r = await api('/adspy/refresh', { method: 'POST' }); $('#cmpMsg') && ($('#cmpMsg').textContent = `✓ อัปเดต ${r.competitors} เพจ — โฆษณาใหม่รวม ${r.newAds} ชิ้น`); }
      catch (e) { alert(e.message); }
      renderAdSpy(main);
    };
    if ($('#cmpAdd')) $('#cmpAdd').onclick = async () => {
      const pageName = $('#cmpName').value.trim();
      if (!pageName) return;
      $('#cmpAdd').disabled = true;
      try {
        await api('/adspy/competitors', { method: 'POST', body: JSON.stringify({ pageName, pageId: $('#cmpPage').value.trim(), country: $('#cmpCountry').value.trim() }) });
        renderAdSpy(main);
      } catch (e) { $('#cmpMsg').textContent = '✕ ' + e.message; $('#cmpAdd').disabled = false; }
    };
  }
  main.querySelectorAll('#adspyTabs button').forEach((b) => b.onclick = () => { a.tab = b.dataset.tab; renderAdSpy(main); });

  renderAdSpyBody($('#adspyBody'), cfg, competitors);
}

async function renderAdSpyBody(box, cfg, competitors) {
  if (!box) return;
  const a = state.adspy;
  const cmpName = a.competitorId ? (competitors.find((c) => c.id === a.competitorId)?.pageName || '') : 'ทุกคู่แข่ง';

  if (a.tab === 'insights') return renderAdSpyInsights(box, cmpName);

  box.innerHTML = '<div class="muted" style="padding:12px">กำลังโหลด…</div>';
  let ads = [];
  try {
    if (a.tab === 'winning') ads = await api('/adspy/winning?limit=30');
    else if (a.tab === 'swipe') ads = await api('/adspy/ads?saved=true&sort=newest' + (a.competitorId ? '&competitorId=' + a.competitorId : ''));
    else ads = await api(`/adspy/ads?sort=${a.sort}&q=${encodeURIComponent(a.q)}` + (a.competitorId ? '&competitorId=' + a.competitorId : ''));
  } catch (e) { box.innerHTML = `<p class="muted">โหลดไม่สำเร็จ: ${esc(e.message)}</p>`; return; }

  const controls = a.tab === 'feed' ? `<div class="ad-controls">
    <input id="adSearch" placeholder="🔍 ค้นหาในข้อความโฆษณา" value="${esc(a.q)}" />
    <select id="adSort">
      <option value="longest" ${a.sort === 'longest' ? 'selected' : ''}>รันนานสุดก่อน</option>
      <option value="winning" ${a.sort === 'winning' ? 'selected' : ''}>คะแนน "เวิร์ก" สูงสุด</option>
      <option value="newest" ${a.sort === 'newest' ? 'selected' : ''}>เจอใหม่ล่าสุด</option>
    </select>
  </div>` : '';

  const hint = a.tab === 'winning'
    ? `<p class="muted">แอดที่รันนานสุด = คู่แข่งยอมจ่ายต่อเนื่อง มักแปลว่าทำกำไร — ตัวที่ควรถอดสูตรมาลอง (${esc(cmpName)})</p>`
    : a.tab === 'swipe' ? `<p class="muted">ครีเอทีฟที่คุณกด ★ เก็บไว้เป็นคลังไอเดีย (${esc(cmpName)})</p>` : '';

  box.innerHTML = `${controls}${hint}
    <div class="ad-grid">${ads.length ? ads.map(adCard).join('') : `<p class="muted">${a.tab === 'swipe' ? 'ยังไม่มีโฆษณาที่บันทึกไว้ — กด ☆ ที่การ์ดใดก็ได้' : 'ยังไม่มีโฆษณา'}</p>`}</div>`;

  if ($('#adSearch')) {
    let t;
    $('#adSearch').oninput = (e) => { clearTimeout(t); const v = e.target.value; t = setTimeout(() => { a.q = v; renderAdSpyBody(box, cfg, competitors); }, 300); };
  }
  if ($('#adSort')) $('#adSort').onchange = (e) => { a.sort = e.target.value; renderAdSpyBody(box, cfg, competitors); };
  box.querySelectorAll('[data-save]').forEach((b) => b.onclick = async () => {
    try { await api('/adspy/ads/' + b.dataset.save + '/save', { method: 'POST', body: JSON.stringify({ saved: b.dataset.next === '1' }) }); }
    catch (e) { return alert(e.message); }
    renderAdSpyBody(box, cfg, competitors);
  });
}

async function renderAdSpyInsights(box, cmpName) {
  const a = state.adspy;
  box.innerHTML = '<div class="muted" style="padding:12px">กำลังวิเคราะห์…</div>';
  let ins;
  try { ins = await api('/adspy/insights' + (a.competitorId ? '?competitorId=' + a.competitorId : '')); }
